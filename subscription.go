...
func GetByIDSubscription(id string, body []byte) (string, error) {
	...
	res, err := do("GET", subscriptionURL+"/"+id, nil, bytes.NewBuffer(body), nil)
	if err != nil {
		return "", err
	}
	response := string(res[:])
...
func GetAllSubscription(queryParams url.Values, body []byte) (string, error) {
	res, err := do("GET", subscriptionURL, queryParams, bytes.NewBuffer(body), nil)
	if err != nil {
		return "", err
	}
	response := string(res[:])
...
func DeleteSubscriptions(id string, body []byte) error {
	...
	_, err := do("DELETE", subscriptionURL+"/"+id, nil, bytes.NewBuffer(body), nil)
	if err != nil {
		return err
	}
